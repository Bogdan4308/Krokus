       ...
       headers = {
		...
		if encoded_content ~= nil then
			...
			if info then
				...
				text_buffer14.v = info.NotWithTimer4
				text_buffer15.v = info.NotWithTimer5
				text_buffer_not_time1.v = info.TimeForNot1
				text_buffer_not_time2.v = info.TimeForNot2
				text_buffer_not_time3.v = info.TimeForNot3
				text_buffer17.v = info.TimeForNot5
				text_buffer16.v = info.TimeForNot5
				...
				local checknotontime3 = info.CheckNotTime3
				local checknotontime4 = info.CheckNotTime4
				local checknotontime5 = info.CheckNotTime5
...
function main()
	...
	sampRegisterChatCommand("pin", function(arg)
		if arg:find("(%w+)_(%w+)") or tonumber(arg) then
			if tonumber(arg) then
				if sampIsPlayerConnected(tonumber(arg)) then
					arg = sampGetPlayerNickname(tonumber(arg))
				else
					arg = mynick
				end
			end
			if arg ~= mynick and arg ~= nil and arg ~= "" then
				DeleteOrAddPlayer(false, arg)
			else
...
function DeleteOrAddPlayer(state_inv, arg)
	...
	if not find_nick and not state_inv then
		table.insert(Fasten, {nick = arg, id = "-1", online = false})
	end
	...
	for k,v in ipairs(Fasten) do
		if Fasten[k].nick then
			if Fasten[k].online then
				renderDrawPolygon(CalculateSizeAndPosX(1650), CalculateSizeAndPosY(350) + k*20+CalculateSizeAndPosY(13), 10, 10, 40, 0, 0xFF00FF00)
				renderFontDrawText(fontupdate, Fasten[k].nick.."["..Fasten[k].id.."]",CalculateSizeAndPosX(1660), CalculateSizeAndPosY(350) + k*20, 0xFFFFFFFF, 0x90000000)
			else
				renderDrawPolygon(CalculateSizeAndPosX(1650), CalculateSizeAndPosY(350) + k*20+CalculateSizeAndPosY(13), 10, 10, 40, 0, 0xFFFF0000)
				renderFontDrawText(fontupdate, Fasten[k].nick,CalculateSizeAndPosX(1660), CalculateSizeAndPosY(350) + k*20, 0xFFFFFFFF, 0x90000000)
			end
		end
...
function sampev.onServerMessage(color, text)
	...
	if text:find("%[Семья (.+)%] "..mynick.."(.+) пригласил в семью нового члена: (%w+_%w+)%[(%d+)%]!") and color == -1178486529 then
		...
		if invid then
			...
			DeleteOrAddPlayer(true, sampGetPlayerNickname(invid))
		end
    end
...
function sampev.onShowDialog(id, st, tytle, button, canc, text)
	...
	if id == 25623 and not_vr then
		sampSendDialogResponse(25623, 1, nil, nil)
		not_vr = false
		return false
	end
	if take_salary then
