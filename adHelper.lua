...
function sampev.onServerMessage(color, text)
    ...
    if text:find("вступить в вашу семью!") then
	    lua_thread.create(function()
			wait(100)
	    	sampSendChat('/b Приглашение в семью отправлено!')
			wait(2000)
	    	sampSendChat('/b нажми английскую X или пропиши команду в чат /offer')
		end)
    end
	...
	if text:find("%[Семья (.+)%] "..mynick.."(.+) пригласил в семью нового члена: (%w+_%w+)%[(%d+)%]!") and color == -1178486529 then
		...
		if invid then
			...
			lua_thread.create(function()
				...
				wait(500)
				sampSendChat("Добро пожаловать в семью!")
				wait(2000)
				sampSendChat("Писать в чат /fam альянс /al")
				wait(2000)
				sampSendChat("Семейное меню /fammenu Инвентарь - Семья")
				wait(2000)
				if invid then
...
function CheckVipPlayers(arg)
	if arg ~= "" and tonumber(arg) and sampIsPlayerConnected(arg) then
		...
		wait(100)
		if findvip then
			sampAddChatMessage("[AdHelper] "..nickcheckvip.." має віп. Ви отримали додатково $250000 до ЗП.", 0x00FF00)
			countVIP = countVIP + 1
			SendMyState()
		end
		wait(500)
	...
	if secondary_window_state.v then
		...
		if imgui.Button(u8"Очистить статистику") then
			if allSum > 10000 then
				...
				--end)
				SendTelegram(mynick.." успешно очистил статистику.\nЗаработано: "..formatNumberWithCommas(allSum).."$\n \n@wmhan1")
				StepAD = 0
	       		StepInv = 0
				minutes = 0
				hour = 0
		...
		if imgui.Button(u8'Заспавнити транспорт') then
			lua_thread.create(function()
			wait(100)
			sampSendChat('/fam :u2757: Спавн фам машин через 10 секунд!')
			wait((10)*1000)
