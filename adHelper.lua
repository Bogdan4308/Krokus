script_author('@BGDN for Ukrainians Family')
...
local text_buffer19 = imgui.ImBuffer(256)
-------------------------------------------------------------------------------
local mynick, dsandtg, GivPlayers, update_link, notebook_text, text_private_notepad = "","","","", "",""
...
local StepAD = 0
local StepInv = 0
local pausedtime, requestTimer, styleimg, countVIP , TimerUpdate, PosPadX, PosPadY, TimeSalary, TimerCheckFmembers = 0,0,0,0,0,0,0,0,0
local StepNot = 0
local Sum = 0
local active, get, notontimer1, notontimer2, paytax,check_tax, take_salary, update_status, numberpl, send_message, state_button_send = false, false,false,false,false,false,false,false,false, true, true
    ...
    asyncHttpRequest("GET", sha_url, {
       headers = {
		...
		if encoded_content ~= nil then
			...
			StepAD = tonumber(info.Ad)
			StepInv = tonumber(info.Inv)
			hour = tonumber(info.Hour)
			minutes = tonumber(info.Minutes)
			dsandtg = info.TextAfterInv
			nickDep1 = info.NickDep1
			nickDep2 = info.NickDep2
			nickDep3 = info.NickDep3
			local info_state_window_main = info.StateMainWindow
			local info_state_window_add = info.StateAddWindow
			if info_state_window_main == "false" then
				checked_time.v = false
			else
				checked_time.v = true
			end
			if info_state_window_add == "false" then
				checked_info9.v = false
			else
				checked_info9.v = true
			end
			text_buffer2.v = info.NotWithTimer1
			text_buffer3.v = info.NotWithTimer2
			text_buffer4.v = info.NotWithTimer3
			text_buffer14.v = info.NotWithTime1
			text_buffer15.v = info.NotWithTime2
			text_buffer16.v = info.TimeForNot1
			text_buffer17.v = info.TimeForNot2
			PosWindowX = info.PosX
			PosWindowY = info.PosY
			local info_check_tax = info.CheckedTax
			if info_check_tax == "true" then
				checked_tax.v = true
			else
				checked_tax.v = false
			end
			styleimg = tonumber(info.WindowStyle)
			ComboStyle.v = styleimg - 1
			local checknotontime1 = info.CheckNotTime1
			local checknotontime2 = info.CheckNotTime2
			if checknotontime1 == "true" then
				notontimer1 = true
			end
			if checknotontime2 == "true" then
				notontimer2 = true
			end
			countVIP = tonumber(info.CountVipPlayers)
			local checktextonthescreen = info.StateNotePad
			if checktextonthescreen ~= nil and checktextonthescreen then
				...
				PosPadX = tonumber(info.PosNotePadX)
				PosPadY = tonumber(info.PosNotePadY)
...
function CheckAddFiles()
	...
	local filesearch_img_question = io.open(getGameDirectory().."\\moonloader\\AdHelper\\image\\question.png")
	if not filesearch_img_question then
		downloadUrlToFile('https://github.com/Bogdan4308/Krokus/blob/Graphics/question.png?raw=true', getGameDirectory().."\\moonloader\\AdHelper\\image\\question.png", function (id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				imageQuestion= imgui.CreateTextureFromFile(getGameDirectory().."\\moonloader\\AdHelper\\image\\question.png")
			end
	end)
	else
		imageQuestion = imgui.CreateTextureFromFile(getGameDirectory().."\\moonloader\\AdHelper\\image\\question.png")
...
function main()
	...
	sampRegisterChatCommand("inv",function(arg) if tonumber(arg) and arg ~= "" and sampIsPlayerConnected(arg) then sampSendChat("/faminvite "..arg) else sampAddChatMessage("[AdHelper] Некоректно введено ID!", 0xFF0000) end end)
	...
	sampAddChatMessage('[AdHelper] @bgdn for Ukrainians Family', 0x00FA9A)
    ...
    elapsedSeconds = currentTime - startTime + tonumber(seconds)
	...
	while true do
		if os.clock() - TimerUpdate > 10*60 then
			CheckUpdate()
		...
		if isKeyJustPressed(VK_W) or isKeyJustPressed(VK_A) or isKeyJustPressed(VK_S) or isKeyJustPressed(VK_D) or isKeyJustPressed(VK_LBUTTON) or isKeyJustPressed(VK_F6) then
			startTimer1()
			if statetime then
        ...
        if notelapsedtime == 0 and state_not1 then
		    state_not = true
		...
		if state_not1 and state_not then  
			...
			if text_buffer2.v ~= "" or  text_buffer3.v ~= "" or  text_buffer4.v ~= "" then
		...
		if os.clock() - TimerCheckFmembers > 1 and fifth_window_state.v then
			sampSendChat("/fmembers")
			check_fmembers = true
			TimerCheckFmembers = os.clock()
		end
...
function DrawOnScreen()
	if checked_time.v then
		...
		renderFontDrawText(font, "{FFFF00}"..StepNot.." КД: "..notelapsedtime,CalculateSizeAndPosX(PosWindowX + 60), CalculateSizeAndPosY(PosWindowY + 127), 0xFFFFFFFF, 0x90000000)	
		renderFontDrawText(font, "{228B22}КД: "..piarelapsedtime, CalculateSizeAndPosX(PosWindowX + 195), CalculateSizeAndPosY(PosWindowY + 127), 0xFFFFFFFF, 0x90000000)	
		...
		if checked_info9.v then
			SizeWindowY = 270
			renderDrawTexture(image8, CalculateSizeAndPosX(PosWindowX + 35), CalculateSizeAndPosY(PosWindowY + 160), CalculateSizeAndPosX(17), CalculateSizeAndPosY(17), 0.0, -1)
			renderDrawTexture(image8, CalculateSizeAndPosX(PosWindowX + 35), CalculateSizeAndPosY(PosWindowY + 185), CalculateSizeAndPosX(17), CalculateSizeAndPosY(17), 0.0, -1)
			renderDrawTexture(image8, CalculateSizeAndPosX(PosWindowX + 35), CalculateSizeAndPosY(PosWindowY + 210), CalculateSizeAndPosX(17), CalculateSizeAndPosY(17), 0.0, -1)
			renderFontDrawText(font, nickDep1..": "..HourDep1..", "..AdDep1..", "..InvDep1, CalculateSizeAndPosX(PosWindowX + 55), CalculateSizeAndPosY(PosWindowY + 160), 0xFFFFFFFF, 0x90000000)
			renderFontDrawText(font, nickDep2..": "..HourDep2..", "..AdDep2..", "..InvDep2, CalculateSizeAndPosX(PosWindowX + 55), CalculateSizeAndPosY(PosWindowY + 185), 0xFFFFFFFF, 0x90000000)
			renderFontDrawText(font, nickDep3..": "..HourDep3..", "..AdDep3..", "..InvDep3, CalculateSizeAndPosX(PosWindowX + 55), CalculateSizeAndPosY(PosWindowY + 210), 0xFFFFFFFF, 0x90000000)
	...
	if checked_notepad.v then
		...
		renderDrawBoxWithBorder(PosPadX, PosPadY + 10, 500, 5, 0x80808080, 0.1, 0x90000000)
...
function cmd_ad(arg)
    sampSendChat('/ad Семья Ukrainians ищет друзей')
	...
	lua_thread.create(function()
		while not getdata do
			...
			function(response)
        		...
        		local info = decodeJson(response.text)
				getdata = true
				if tonumber(info.day_of_week) == 0 and allSum >= 1000000 then
...
function imgui.OnDrawFrame()
	...
	if fifth_window_state.v then
	    ...
	    imgui.Begin(u8"[AdHelper] | Керування | Ukrainians Family", fifth_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
		imguiDrawHeaders()
		...
		local countplayerswithscrpt = 0
			for k, v in ipairs(members) do
				...
				imgui.TextColoredRGB("{"..members[k].color.."}("..members[k].rank..") "..members[k].nick.."["..members[k].idp.."]".." ["..members[k].lvl.."]"..members[k].quest.."/8 ("..members[k].afk..")") 
				for ku, vu in ipairs(Users) do
					if string.match(Users[ku].flpatch , members[k].nick) then
						countplayerswithscrpt = countplayerswithscrpt+ 1
						imgui.SameLine()
						imgui.Image(imageUsers, imgui.ImVec2(20, 20))
						if imgui.IsItemClicked() then
							--sampAddChatMessage('Ви натиснули на іконку', -1)
						end
					end
				end
				...
				imgui.PopFont()
				imgui.SameLine()
				if imgui.Button(u8"Набрати##"..members[k].idp) then
					sampSendChat("/number "..members[k].idp)
					numberpl = true
	...
	if third_window_state.v then
		...
		imgui.SetNextWindowPos(imgui.ImVec2(sw/ 2 , sh / 2 ), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(sw/2, sh/2), imgui.Cond.FirstUseEver)
	    imgui.Begin(u8"[AdHelper] | Активність | Ukrainians Family", third_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
		imguiDrawActive()
	...
	if main_window_state.v then
	    ...
	    imgui.Begin(u8"[AdHelper] | Налаштування | Ukrainians Family", main_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
...
function ImguiMAINWindow()
	if secondary_window_state.v then
	    ...
	    imgui.Begin(u8"[AdHelper] | Привіт, "..mynick.."| Ukrainians Family", secondary_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
		...
		imgui.InputTextWithHint('  ',u8"Введіть текст, який буде надіслано в сімейний чат.", text_buffer)	
		imgui.SameLine()
		if imgui.Button(u8'Зберегти') then
		...
		if checked_piar.v then
			...
			if not render_state and active then
		...
		if checked_not.v then
			...
			imgui.SameLine()
			imgui.PushItemWidth(250)
			imgui.InputText(u8"1 рядoк", text_buffer14)
			imgui.SameLine()
			imgui.PushItemWidth(60)
			imgui.InputText(u8"Хв і сек в форматі: 00:00", text_buffer16)
			imgui.SameLine()
			if not notontimer1 then
				if imgui.Button(u8"Старт##1") then
					minutesnot1, secondsnot1 = string.match(text_buffer16.v, "(%d+):(%d+)")
					if string.match(text_buffer16.v, "^(%d+):(%d+)$") and tonumber(minutesnot1) < 60 and tonumber(minutesnot1) >= 0 and tonumber(secondsnot1) < 60 and tonumber(secondsnot1) >= 0  then
			...
			imgui.SameLine()
			imgui.PushItemWidth(250)
			imgui.InputText(u8"2 рядoк", text_buffer15)
			imgui.SameLine()
			imgui.PushItemWidth(60)
			imgui.InputText(u8"Хв і сек в фoрматі: 00:00", text_buffer17)
			imgui.SameLine()
			if not notontimer2 then
				if imgui.Button(u8"Стaрт##2") then
					minutesnot2, secondsnot2 = string.match(text_buffer17.v, "(%d+):(%d+)")
					if string.match(text_buffer17.v, "^(%d+):(%d+)$") and tonumber(minutesnot2) < 60 and tonumber(minutesnot2) >= 0 and tonumber(secondsnot2) < 60 and tonumber(secondsnot2) >= 0  then
				...
				if imgui.Button(u8"Стoп##2") then
					notontimer2 = false
				end
			end
			imgui.PushItemWidth(250)
		...
		if checked_time.v then
			...
			if checked_info9.v then
				...
				imgui.InputTextWithHint(u8"##5",u8"Введіть нік зама", text_buffer18)
				imgui.SameLine()
		...
		imgui.SameLine(CalculateSizeAndPosX(925))
		imgui.Image(imageQuestion, imgui.ImVec2(20, 20))
		if imgui.IsItemClicked() then
			lua_thread.create(function()
...
function imguiDrawHeaders()
	...
	if imgui.ButtonClickable(state_btn_logs, u8"Логи", imgui.ImVec2(CalculateSizeAndPosX(220), CalculateSizeAndPosY(30))) then
	...
	imgui.SameLine()
	if imgui.ButtonClickable(state_btn_drive, u8"Керування", imgui.ImVec2(CalculateSizeAndPosX(220), CalculateSizeAndPosY(30))) then
		members = {}
		Users = {}
		DepActive = {}
		InfoFamily = {}
	...
	imgui.SameLine()
	if imgui.ButtonClickable(state_btn_notpad, u8"Блокнот", imgui.ImVec2(CalculateSizeAndPosX(220), CalculateSizeAndPosY(30))) then
		seventh_window_state.v = false
		secondary_window_state.v = false
		fifth_window_state.v = false
	...
	imgui.SameLine()
	if imgui.ButtonClickable(state_btn_active, u8"Активність", imgui.ImVec2(CalculateSizeAndPosX(220), CalculateSizeAndPosY(30))) then
		members = {}
		Users = {}
		DepActive = {}
		InfoFamily = {}
...
function imguiDrawLogs() 
	if seventh_window_state.v then
	    ...
	    imgui.SetNextWindowPos(imgui.ImVec2(sw/ 2 , sh / 2 ), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(sw/2, sh/2), imgui.Cond.FirstUseEver)
	    imgui.Begin(u8"[AdHelper] | Логи | Ukrainians Family", seventh_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
		imguiDrawHeaders()
		
		if imgui.BeginChild('Name', imgui.ImVec2((sw/2) - CalculateSizeAndPosX(25), (sh/2) - CalculateSizeAndPosY(130)), true) then
			imgui.Text(base64.decode(logs_money))
            imgui.EndChild() 
        end
		...
		if imgui.Button(u8"Вигрузити лог") then
			...
			if file then
				file:write(base64.decode(logs_money))
				file:close()
			end
...
function imguiDrawTasks()
	if sixth_window_state.v then
		imgui.SetNextWindowPos(imgui.ImVec2(sw/ 2 , sh / 2 ), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(sw/2, sh/2), imgui.Cond.FirstUseEver)
		imgui.Begin(u8"[AdHelper] | Завдання | Ukrainians Family", sixth_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
		imguiDrawHeaders()
		if imgui.BeginChild('Name', imgui.ImVec2(CalculateSizeAndPosX(600), CalculateSizeAndPosY(200)), true) then
		...
		if imgui.BeginPopupModal(u8'Налаштування##2', _, imgui.WindowFlags.NoResize) then
			...
			if checked_notepad.v then 
				...
				imgui.SliderInt(u8"Розмір шрифта", slider_font_size, 0, 20)
				font_notepad = renderCreateFont("Calibri", slider_font_size.v, slider_font_family.v)
				imgui.SameLine()
...
function sampev.onShowDialog(id, st, tytle, button, canc, text)
	...
	if not sampIsDialogActive() and paytax then
		if currentIdDialog == 2763 then
			local taxonfam = string.match(textDialog, "Оплатить налог на квартиру %[%$(.+) /")
			if taxonfam then
	...
	if not sampIsDialogActive() then --piar1
		if id == 25477 then 
	...
	if check_fmembers then
		if not sampIsDialogActive() then
			if tytle:find('{BFBBBA}Члены семьи онлайн') and st == 2 then
				sampSendDialogResponse(id, 1, 0, nil)
			end
			if tytle:find('{BFBBBA}{FFFFFF}.+%(В сети: %d+%) | {ae433d}Семья') and st == 5 then
				local next_page = 0
				local count = 0
