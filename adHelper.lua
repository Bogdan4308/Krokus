script_author('Budanov for Krokus Family')
...
-------------------------------------------------------------------------------
local mynick, dsandtg, GivPlayers, update_link = "","","",""
...
local StepAD = 0
local StepInv = 0
local pausedtime, requestTimer, styleimg, countVIP , TimerUpdate= 0,0,0,0,0
local StepNot = 0
local Sum = 0
local active, get, notontimer1, notontimer2, paytax,check_tax, take_salary, update_status = false, false,false,false,true,false,false,false
...
function asyncHttpRequest(method, url, args, resolve, reject)
	if os.clock() - requestTimer > 5 then
    ...
    asyncHttpRequest("GET", sha_url, {
       headers = {
		...
		if encoded_content ~= nil then
			...
			print(u8:decode(base64.decode(contentfilesinfo)))
...
function main()
	...
	texture = renderLoadTextureFromFile(getGameDirectory() .. '\\moonloader\\AdHelper\\Banderas.jpg')
	...
	sampAddChatMessage('[AdHelper] @bgdn for Krokus Family', 0x00FA9A)
		...
		if response then
			if string.match(response.text, "update_link") then
				...
				if tonumber(version) > tonumber(script__version) then
					update_status = true
					sampAddChatMessage("[AdHelper] Доступне оновлення. Версія: "..version, 0x8A2BE2)
...
function DrawOnScreen()
	if checked_time.v then
		...
		if checked_info9.v then
			SizeWindowY = CalculateSizeAndPosY(270)
			...
			SizeWindowY = CalculateSizeAndPosY(180)
		end
	end
end
...
function sampev.onServerMessage(color, text)
	if text:find("%[Семья%](.+)(%w+_%w+)%[(%d+)%](.+) "..mynick.." інвайт (%d+)") then
	...
	if text:find("%[Семья%](.+)(%w+_%w+)%[(%d+)%](.+) "..mynick.." кік (%d+) (.+)") then
	...
	if text:find("%[Семья%](.+)(%w+_%w+)%[(%d+)%](.+) "..mynick.." ранг (%d+) (%d+)") then
	...
	if text:find("%[Семья%](.+)(%w+_%w+)%[(%d+)%](.+) "..mynick.." мут (%d+) (%d+) (.+)") then
	...
	if text:find("%[Семья%](.+)(%w+_%w+)%[(%d+)%](.+) "..mynick.." розмут (%d+)") then
	...
	if nick1 then
		...
		if tonumber(fammoney) >= 500000 then
			lua_thread.create(function ()
				GetLogMoney(u8(os.date('%x [%X] ', timehs)..nick1.." поклав "..input1))
			end)
		end
	...
	if nick2 then
		...
		if tonumber(fammoney) >= 500000 then
			lua_thread.create(function ()
				GetLogMoney(u8(os.date('%x [%X] ', timehs)..nick2.." взяв "..input2))
			end)
		end
	...
	if text:find("%[Семья (.+)%] "..mynick.."(.+) пригласил в семью нового члена: (%w+_%w+)%[(%d+)%]!") then
	...
	if text:find(mynick..'(%[(%d+)%]):{FFFFFF} Взял $(.+) со склада семьи!') then
		...
		if money_salary then
			...
			if tonumber(money_salary) == allSum then
				StepAD = 0
	       	 	StepInv = 0
				minutes = 0
				hour = 0
...
function cmd_ad(arg)
    sampSendChat('/ad Семья Krokus ищет друзей')
...
function imgui.OnDrawFrame()
	...
	if secondary_window_state.v then
	    ...
	    imgui.Begin(u8"[AdHelper] | Привіт, "..nick.."| Krokus Family", secondary_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
		...
		imgui.SameLine()
		if imgui.Button(u8"Чорний список", imgui.ImVec2(220, 30)) then
		    fifth_window_state.v = true
			secondary_window_state.v = false
			imgui.Process = fifth_window_state.v
		end
		...
		if checked_time.v then
			...
			if checked_info9.v then
				...
				imgui.InputTextWithHint(u8"    ",u8"Введіть нік зама", text_buffer18)
				imgui.SameLine()
				imgui.SetCursorPosX(sw/6)
				if imgui.Button(u8"Добaвити") then
					lua_thread.create(function ()
						CheckAddDeputy()
					end)
				end
	...
	if main_window_state.v then
	    ...
	    imgui.Begin(u8"[AdHelper] | Налаштування | Krokus Family", main_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
	...
	if seventh_window_state.v then
	    ...
	    imgui.SetNextWindowPos(imgui.ImVec2(sw/ 2 , sh / 2 ), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(sw/2, sh/2), imgui.Cond.FirstUseEver)
	    imgui.Begin(u8"[AdHelper] | Логи | Krokus Family", seventh_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
		if imgui.Button(u8"Основна", imgui.ImVec2(220, 30)) then
		    seventh_window_state.v = false
			secondary_window_state.v = true
		
		end
		if imgui.BeginChild('Name', imgui.ImVec2((sw/2) - 25, (sh/2) - 90), true) then
...
function CheckAddDeputy()
	if  nickDep1 ~= text_buffer18.v and nickDep2 ~= text_buffer18.v and nickDep3 ~= text_buffer18.v then
		if os.clock() - TimerReq > 10 then
			if nickDep1 ~= "" and nickDep2 ~= "" and nickDep3 ~= "" then nickDep1 = "" end
				AddDeputy(u8:decode(text_buffer18.v))
				TimerReq = os.clock()
			else
...
function checkTax()
	if currentIdDialog == 2763 then
		...
		if taxonfam then
			tax = taxonfam
			if string.match(tax, "%p") then
				tax = string.gsub(tax, "%p","")
			end
...
function sampev.onShowDialog(id, st, tytle, button, canc, text)
	...
	if id == 2763 and take_salary then
		sampSendDialogResponse(2763, 1, 5, nil)
	end
	if id == 2765 and take_salary then
		sampSendDialogResponse(2765, 1, nil, allSum)
		take_salary = false
	end
